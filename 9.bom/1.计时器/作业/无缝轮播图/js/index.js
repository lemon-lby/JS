...
function switchTo(index, direction) {
    ...
    function animateSwitch() {
        ...
        //2. 计算总距离
        var distance, marginLeft = parseFloat(getComputedStyle(config.doms.divImgs).marginLeft),
        ...
        config.timer.id = setInterval(function () {
            ...
            if (direction === "right" && Math.abs(marginLeft) > totalWidth) {
                marginLeft += totalWidth;
            } else if (direction === "left" && Math.abs(marginLeft) < config.imgWidth) {
                marginLeft -= totalWidth;
            }
